...
export const DesktopSidebar = ({
  ...
  return (
    <>
      <motion.div
        className={cn(
          "fixed left-0 top-0 z-10 h-screen px-4 py-4 hidden md:flex md:flex-col bg-card border-r border-border w-[300px] shrink-0",
          className
        )}
        animate={{
...
export const SidebarLink = ({
  ...
  if (asChild) {
    return (
      <div
        className={cn(
          "flex items-center justify-start gap-2 group/sidebar py-2 rounded-md px-2 hover:bg-accent transition-colors cursor-pointer",
          className
        )}
        onClick={handleClick}
        {...props}
  ...
  // For internal routes, we'll use a div and let the parent handle navigation
  return (
    <div
      className={cn(
        "flex items-center justify-start gap-2 group/sidebar py-2 rounded-md px-2 hover:bg-accent transition-colors cursor-pointer",
